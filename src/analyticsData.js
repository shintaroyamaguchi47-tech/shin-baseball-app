...
export function buildAnalytics(db, filters = {}) {
  ...
  const runsAllowed=runsAgainst;
  return { team, games, wins, losses, runsFor, runsAgainst, innings:[...inningMap.values()], gameFactors, batting:{PA:battingPa.length,AB,H,BB,K,OBP:rate(H+BB,battingPa.length),SLG:rate(TB,AB),OPS:(Number(rate(H+BB,battingPa.length))+Number(rate(TB,AB))).toFixed(3),KPct:pct(K,battingPa.length),BBPct:pct(BB,battingPa.length)}, pitching:{PA:pitchingPa.length,H:pitchingPa.filter(pa=>pa.isHit).length,BB:pitchingPa.filter(pa=>pa.isWalk).length,K:pitchingPa.filter(pa=>pa.isStrikeout).length,AVG:rate(pitchingPa.filter(pa=>pa.isHit).length,pitchingPa.filter(pa=>pa.isAtBat).length),WHIP:rate(pitchingPa.filter(pa=>pa.isHit||pa.isWalk).length,inningsPitched),ERA:rate(runsAllowed*7,inningsPitched,2),IP:inningsPitched.toFixed(1),firstStrikePct:pct(firstStrikes,first.length)}, fielding:{errors},running:{attempts:steals.length,success:steals.filter(e=>e.success).length,successPct:pct(steals.filter(e=>e.success).length,steals.length)}, players:playerRows, patterns };
}
