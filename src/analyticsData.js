...
// ヒット判定は試合中集計(App.jsxのcalcBatting)と揃える。
// 実際の記録は守備位置を頭に付けた文字列(例「遊安」「中二塁打」)なので部分一致で見る。
// 単打は「安打」ではなく「安」で記録されるため'安'で拾う('塁打'は二/三/本塁打を包含)。
const isHit = result => ['安', '塁打', '本塁打'].some(v => result?.includes(v));
const bases = result => result?.includes('本塁打') ? 4 : result?.includes('三塁打') ? 3 : result?.includes('二塁打') ? 2 : isHit(result) ? 1 : 0;
// OBP(出塁率)の分母。犠打は分母から除外し犠飛は含める(公認野球規則の定義に準拠)。
const obDen = list => list.filter(pa => !pa.isSacBunt).length;
...
export function normalizeArchive(savedGames = [], registeredTeams = [], homeTeamName = '') {
  ...
  savedGames.forEach((game, gi) => {
    ...
    const flush = () => {
      ...
      // 打者が生きて出塁した打席(安打・四死球・失策・振り逃げ生還)は守備のアウトを伴わない。
      // 併殺打は2アウト。それ以外はアウト1として投球回(IP)を集計する。
      const reachedSafe = hit || walk || /エラー|失策/.test(result) || (result.includes('振り逃げ') && !result.includes('振り逃げアウト'));
      const outsMade = result.includes('併殺') ? 2 : reachedSafe ? 0 : 1;
      plateAppearances.push({ id: paId, gameId, inning: first.inning, half: first.isTop ? 'top' : 'bottom', battingTeamId: teams.get(battingTeam)?.id, fieldingTeamId: teams.get(fieldingTeam)?.id, batterId, pitcherId, batterName: last?.batterName || '不明', pitcherName: last?.pitcherName || '不明', batterBats: last?.batterBats || '', pitcherThrows: last?.pitcherThrows || '', battingOrder: last?.batter, result, isHit: hit, isWalk: walk, isStrikeout: strikeout, totalBases: bases(result), isAtBat: !walk && !result.includes('犠'), isSacBunt: result.includes('犠打'), outsRecorded: outsMade, pitches: nonEvents.length, countBeforeResult: last?.count || null });
...
export function buildAnalytics(db, filters = {}) {
  ...
  const playerRows = db.players.filter(p=>p.teamId===team.id).map(player => { const bp=battingPa.filter(pa=>pa.batterId===player.id), pp=pitchingPa.filter(pa=>pa.pitcherId===player.id), ab=bp.filter(pa=>pa.isAtBat).length,h=bp.filter(pa=>pa.isHit).length,bb=bp.filter(pa=>pa.isWalk).length,tb=bp.reduce((n,pa)=>n+pa.totalBases,0),outs=pp.reduce((n,pa)=>n+(pa.outsRecorded||0),0),ip=outs/3; const running=db.baserunningEvents.filter(e=>gids.has(e.gameId)&&e.playerId===player.id), fielding=db.fieldingEvents.filter(e=>gids.has(e.gameId)&&e.playerId===player.id); return {...player, PA:bp.length,AB:ab,H:h,BB:bb,AVG:rate(h,ab), OBP:rate(h+bb,obDen(bp)), SLG:rate(tb,ab), OPS:(Number(rate(h+bb,obDen(bp)))+Number(rate(tb,ab))).toFixed(3), K:bp.filter(pa=>pa.isStrikeout).length, pitchingPA:pp.length, HAllowed:pp.filter(pa=>pa.isHit).length, BBAllowed:pp.filter(pa=>pa.isWalk).length, SO:pp.filter(pa=>pa.isStrikeout).length,IP:ip.toFixed(1),WHIP:rate(pp.filter(pa=>pa.isHit||pa.isWalk).length,ip),SB:running.filter(e=>e.success).length,CS:running.filter(e=>!e.success).length,E:fielding.length}; }).filter(p=>p.PA||p.pitchingPA||p.SB||p.CS||p.E);
